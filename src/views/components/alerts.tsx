...
const Alert = ({theme, title, description}: IProps) => {
        ...
        <>
            ...
            <div className={`relative flex flex-col sm:flex-row sm:items-center ${AlertTheme.backgroundColor} transition duration-400 ease-in-out shadow rounded-md py-5 pl-6 pr-8 sm:pr-6`}>
                <div className="flex flex-row items-center border-b sm:border-b-0 w-full sm:w-auto pb-4 sm:pb-0">
                    ...
                    <div className="text-sm font-medium ml-3">{title ?? AlertTheme.title}</div>
                </div>
                {AlertTheme.description ?
                <div className="text-sm tracking-wide text-gray-500 mt-4 sm:mt-0 sm:ml-4">{description ?? AlertTheme.description}</div>
                : null}
